...
//--------------------------
	 ...
	 * 
	 */
	public void searchContactsTable() {
		int numberOfContactsFound = 0;
		List <Person> tempArray = new ArrayList<Person>();
		...
		if (contactCounter == 0) {
			...
			for (int counter = 0; counter < contactCounter; counter++) {
				if (lastName.equalsIgnoreCase(contactList[counter]
						.getLastName())) {
					tempArray.add(contactList[counter]);
			...
			if (numberOfContactsFound == 0) {
				...
				Person[] searchedContacts = new Person[tempArray.size()];
				searchedContacts = tempArray.toArray(searchedContacts);
				Table.printTop();
				
				int numberOfContacts = searchedContacts.length;
				
				for (int index = 0; index < numberOfContacts; index++) {
					Table contactToPrint = new Table(searchedContacts[index], index);
